        ...
        this.template = [
            {
                ...
                children: [
                    ...
                    { text: "导入用户", name: "import", dbid: 6 },
                    { text: "修改其他用户角色", name: "attachRole", dbid: 7 },
                ],
            },
            {
                text: "竞赛管理权限(学生竞赛与教师获奖)",
                name: "contest",
                children: [
                    {
                        text: "查询/搜索",
                        ...
                        children: [
                            ...
                            { text: '奖项', name: 'prize' },
                            { text: '级别', name: 'level' },
                            { text: '获奖时间', name: 'award_time' },
                            { text: '获奖证书', name: 'certificate' },
                            { text: '登记时间', name: 'add_time' },
                            { text: '登记人', name: 'add_staff_info_id' }
                        ]
                    },
                    { text: "插入/添加", name: "insert", dbid: 2 },
                    { text: "删除", name: "delete", dbid: 3 },
    ...
    methods: {
        ...
        collect() {
            ...
            for (let table of this.processedInitialList) {
                ...
                if (table.checked === 0 || table.checked == null) continue;
                for (let operation of table.children) {
                    // skip digging deeper if operation is unchecked
                    if (operation.checked === 0 || operation.checked == null) continue;
        ...
        removeRedundant(list) {
            ...
            if (asterItems.length === 0) return result;
